...
class ConfigFile():
    ...
    def GetConfiguration(self,field)->dict:
        
        if type(field) is not str:
            self.LogMessage(f"GetConfiguration --> wrong data type ,  field must be str","warning")
            return False
        ...
        try:
            ...
            result=self.__configuration.get(field)
            
            if result==None:
                self.LogMessage(f"GetConfiguration --> configuration field {field} not found in configuration","warning")
                return False
            ...
            self.LogMessage(f"GetConfiguration --> Configuration data not found","warning")
            return False
